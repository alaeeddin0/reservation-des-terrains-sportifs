                                    ...
                                    <tbody>
                                        <?php foreach ($terrains as $terrain): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <form action="/Terrain/update/<?= $terrain['id']; ?>" method="post"
                                                        style="display:inline;">
                                                        <?= csrf_field(); ?>
                                                        <input type="text" name="prix" value="<?= esc($terrain['prix']) ?>"
                                                            class="form-control form-control-sm d-inline-block"
                                                            style="width: 70px;" placeholder="Prix">

                                                        <button type="submit" class="btn btn-danger btn-sm"><i
                                                                class="bi bi-pencil-square"></i></button>
                                                    </form>
                                                    <form action="/Terrain/delete/<?= $terrain['id']; ?>" method="post"
                                                        style="display:inline;">
                                                        <?= csrf_field(); ?>
                                                        <button type="submit" class="btn btn-danger btn-sm"
                                                            onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce terrain ?');"><i
